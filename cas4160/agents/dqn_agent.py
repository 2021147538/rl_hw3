...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        if np.random.rand() < epsilon:
            action = torch.randint(0, self.num_actions, size=(1,), device=ptu.device)
        else:
            with torch.no_grad():
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action = next_action.long().view(-1, 1)
            next_q_values = torch.gather(next_qa_values, 1, next_action).squeeze(1) # see torch.gather
            target_values = reward + self.discount * (1.0 - done) * next_q_values
